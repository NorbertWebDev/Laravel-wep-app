...
The required steps for the database so use the following steps:
    ...
    2. Import the database structure from the Database folder using db.sql file.
        ...
        2.0 At first the Laravel's version must be checked, to be sure you have the required to be able to use
            this web app
        2.1 See the connection strings in the loadConnectionValues method in the Database folder's MySqlController.php
	...
	Run the fill up of the menuactive(used by the web app to display the menu) in the database
	------------------------------------------------------------------------------------------
...
Thank you for using the sample Laravel web app!
